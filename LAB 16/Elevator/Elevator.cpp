...
bool Elevator::isNearDestination() const
{
  ...
  if(direction == UP)
  {
    ...
    if(location >= 0)
    {
      if(abs((toFloor->getLocation()) - location) <= speed)return true;
    }
    if(location < 0)
    {
      if(abs((toFloor->getLocation()) - location) <= speed)return true;
  ...
  if(direction == DOWN)
  {
    ...
    if(location > 0)
    {
      if(abs(location - (toFloor->getLocation())) <= speed)return true;
    }
    if(location < 0)
    {
      if(abs((toFloor->getLocation()) - location) <= speed)return true;
    ...
    if(location == 0)
    {
      if(location == (toFloor->getLocation()))return true;
    }
  }
...
vector<Rider> Elevator::removeRidersForDestinationFloor()
{
  ...
  if(riders.size() != 0)
  {
    ...
    for(int i = 0; i < riders.size(); i++)
    {
      ...
      if(riders[i].getDestination().getLocation() == location)
...
void Elevator::setDestinationBasedOnRiders()
{
  ...
  for(int i = 0; i < riders.size(); i++)
  {
    ...
    int temp = (riders[i].getDestination().getLocation() - abs(location));/// convert to absolute value
